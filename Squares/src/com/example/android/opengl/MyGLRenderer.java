...
public class MyGLRenderer implements GLSurfaceView.Renderer {
	...
	private int[][] markedSquares = new int[9][9];
	private float mX = 2;
	private float mY = 2;
	...
	public MyGLRenderer() {
		...
		for (int i = 0; i < markedSquares.length; i++) {
			for (int j = 0; j < markedSquares[i].length; j++) {
				markedSquares[i][j] = 0;
			}
		}
		currentPlayer = true;
	...
	public void onSurfaceCreated(GL10 gl, EGLConfig config) {
		...
		for (int i = 0; i < 9; i++) {
			for (int j = 0; j < 9; j++) {
				markSquare[i][j] = new Square(-0.7f + i * 1.4f / 9,
						0.4f - j * 1.4f / 9, 1.4f / 9f, -1.4f / 9f);
			}
		}

		for (int i = 0; i < 10; i++) {
			mSquare[i][0] = new Square(-0.7f, 0.4f - i * 1.4f / 9, 1.4f, 0.01f);
			mSquare[i][1] = new Square(-0.7f + i * 1.39f / 9, -1.0f, 0.01f,
					1.4f);
		}
	...
	public void onDrawFrame(GL10 gl) {
		...
		for (int i = 0; i < 9; i++)
			for (int j = 0; j < 9; j++)
				if (mX > -0.7 + +i * 1.39f / 9
						&& mX < -0.7f + 1.39f / 9 + i * 1.39f / 9)
					if (mY < 0.4f - j * 1.4f / 9
							&& mY > 0.4f - 1.4f / 9 - j * 1.4f / 9) {
						if (markedSquares[i][j] == 0) {
							int score = 0;
							try {
								score = board.put(i, j);
								Log.d(tag, "Put on " + i + "+" + j);
								Log.d(tag, "Scores: " + score);
							} catch (Exception e) {
								Log.e(tag, "Failed to put on " + i + "+" + j, e);
							...
							Log.d(tag, "P1scores: " + player1score);
							Log.d(tag, "P2scores: " + player2score);
						}
					}

		for (int i = 0; i < 9; i++) {
			for (int j = 0; j < 9; j++) {
				if (markedSquares[i][j] == 1)
					gl.glColor4f(0.98f, 0.98f, 0.12f, 1.0f);
				else if (markedSquares[i][j] == 2)
